...
(** A stream is conceptually a possibly-unbounded list of states. *)
...
(** A state is a pair of a substitution (represented as an asso-
ciation list) and a non-negative integer representing a fresh-
variable counter (last being index of the next variable to create) *)
...
let rec
    (** disj returns a non-empty stream if either of its two arguments are successful *)
    disj (g1: goal) (g2: goal) (t: state) = mplus (g1 t) (g2 t) and
    (** conj returns a non-empty stream if the second argument can be achieved in the stream generated by the first *)
    conj (g1: goal) (g2: goal) (t: state) = bind g2 (g1 t) and 
    (** a goal that always fails *)
    null (s: state) = Mzero  and
    (** a goal that always succeeds *)
    nuts (s: state) = just s and
...
(** user-level alias for a state virtually devoid of information: *)
